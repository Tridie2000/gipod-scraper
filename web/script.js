...
function handleClosureClick(closure, map) {
  ...
  toggleButton.onclick = async () => {
    ...
    if (map.getLayer(`closure-${closure.id}`)) {
      map.setPaintProperty(`closure-${closure.id}`, 'fill-color', updatedClosure.handled ? '#888888' : '#FF0000');
    }
    initializeMap();
  };
...
function zoomToPolygon(map, closure) {
  ...
  geojsonData.forEach((polygon) => polygon.forEach((ring) => ring.forEach(([lng, lat]) => bounds.extend([lng, lat]))));
